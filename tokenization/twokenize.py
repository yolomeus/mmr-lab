...
def fe_net_tokenize(text):
    """Extends the twokenizer to separate # and @ from following words as described in the FENet paper.

    :param text: tweet text.
    :return: list of tokens.
    """
    tokens = tokenize(normalizeTextForTagger(text))
    # adapt to FENet paper
    new_tokens = []
    for token in tokens:
        if len(token) > 1:
            if token.startswith('#'):
    ...
    return map(str.lower, new_tokens)
